...
def get_avg(queue: List[float]) -> float:
    ...
    if s >= settings.max_limit:
        return 1.0
    # 中间区域线性插值: [min_limit, max_limit] -> [min_power, 1]
...
async def get_patterns_detail():
    """
    返回所有 patterns 的详细波形数据，用于前端可视化预览。
    每个 pattern 是 [[pulse_ms, pause_ms, amplitude], ...]，amplitude 范围 0-31。
    """
    try:
...
@router.get("/pattern/detail")
async def get_pattern_detail(name: str):
    """
    获取单个 pattern 的详细波形数据。
    """
    try:
        if name not in ci.patterns:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Pattern {name} not found",
            )
        return {"name": name, "pattern": ci.patterns[name]}
